...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% PLOTTING MEAN OF POWER SIGAL AGAINST SPACING
% LOOPED FOR ALL CENTER TURBINES
% PLOTTED AS A TILEDLAYOUT
% NON-NORMALIZED
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for c = 1:length(centers)
    ...
    h(c) = nexttile;
    title(sprintf('Row %1.0f', ceil(turbine/3)))
    hold on
    for st = 1:length(wave_steepnesses)
        wave_steepness = wave_steepnesses(st);
        steep = compose('%02d', round(100 * wave_steepness));
        ...
        for w = 1:length(wavelengths)
            ...
            for s = 1:length(farm_spacings)
                ...
                scatter(farm_spacings(s), data, sz, spacing_shapes{s}, 'filled', ...
                        'MarkerFaceColor', colors(w, :), 'MarkerFaceAlpha', steepness_alpha(st), ...
                        'HandleVisibility', 'off')
...
for c = 1:length(centers)
    ...
    for st = 1:length(wave_steepnesses)
        ...
        for w = 1:length(wavelengths)
            ...
            for s = 1:length(farm_spacings)
                ...
                harmonic_ratio = farm_spacings(s) / wavelengths(w);
                data = averages.(farm_spacing)(turbine).(wave);
                data_no_wave = averages.(farm_spacing)(turbine).("LM0_AK00");

                tmpX(s) = harmonic_ratio;
                tmpY(s) = data;

                scatter(farm_spacings(s), data / data_no_wave, sz, spacing_shapes{s}, 'filled', ...
                        'MarkerFaceColor', colors(w, :), 'MarkerFaceAlpha', steepness_alpha(st), ...
                        'HandleVisibility', 'off')
    ...
    xlabel('$S_x / D$', 'interpreter', 'latex')
    xlim([2.8, 5.2])
    yline(1, 'LineStyle', '--', 'color', 'black', 'alpha', 0.5, 'linewidth', 1, 'HandleVisibility', 'off')
    hold off    
    axis square
end
...
linkaxes(h, 'xy')
ylim([0.5, 1.5])
sgtitle(sprintf('%s %s: Relative Mean Power', farm_arrangement, fancy_name), 'Interpreter', 'latex')
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% PLOTTING MEAN OF POWER SIGAL AGAINST SPACINGS
% LOOPED FOR ALL CENTER TURBINES
% PLOTTED AS A TILEDLAYOUT
% NORMALIZED BY POWER UNDER NO-WAVES
% HIGHLIGHTING THE GAINS/LOSS FROM THE NO-WAVE
    ...
    for st = 1:length(wave_steepnesses)
        ...
        for w = 1:length(wavelengths)
            ...
            for s = 1:length(farm_spacings)
                ...
                scatter(farm_spacings(s), power_gain_loss, sz, spacing_shapes{s}, 'filled', ...
                        'MarkerFaceColor', colors(w, :), 'MarkerFaceAlpha', steepness_alpha(st), ...
                        'HandleVisibility', 'off')
            end
        end
